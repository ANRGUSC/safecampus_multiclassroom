...
class PPOAgent:
    def __init__(self, agents, state_dim, action_space_size, learning_rate=0.00001, discount_factor=0.99, clip_epsilon=0.2):
        self.agents = agents
    ...
    def update(self, agent, trajectory, old_log_probs):
        ...
        # Compute critic loss (MSE between state values and rewards)
        critic_loss = 0.5 * (rewards - state_values).pow(2).mean()
    ...
    def train(self, env, max_steps=30, epochs=4):
        pbar = tqdm(total=1500, desc="Training Progress", leave=True)
