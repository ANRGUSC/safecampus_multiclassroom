...
class MultiClassroomEnv(ParallelEnv):
    def __init__(self, num_classrooms=1, total_students=100, max_weeks=2, action_levels_per_class=None,
                 alpha=0.008, beta=0.01, phi=0.5, gamma=0.3, seed=None):
    ...
    def reset(self, seed=None, options=None):
        if seed is not None:
            self.seed(seed)  # Set the seed if provided

        self.student_status = [np.random.randint(0, 50) for _ in
                               range(self.num_classrooms)]  # Random initial infected per classroom
        ...
        # Ensure _get_observations returns a dictionary
        if observations is None:
            raise ValueError("The observations dictionary returned by _get_observations() is None.")
