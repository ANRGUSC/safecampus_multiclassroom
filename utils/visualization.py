...
def visualize_dqn_states(model, env):
    """
    Visualize the state-action mappings for all agents and classrooms in the environment.
    Args:
        model: Trained model to be used for action prediction.
        env: The environment to visualize.
    """
    method_name = "viz_all_states"
    ...
    for agent in env.agents:
        ...
        for infected in range(0, env.total_students + 1, 10):  # Increment by 10 for clarity in visualization
            for community_risk in range(0, 101, 10):  # Community risk from 0% to 100%
                state = torch.FloatTensor([infected, community_risk / 100]).unsqueeze(0)  # Only use infected and community_risk as input

                # Forward pass through the model to get action predictions
                with torch.no_grad():
                    q_values = model(state)  # Directly use the passed `model` instance here
        ...
        ax.set_title(f'State Visualization for {agent}')
